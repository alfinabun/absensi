    ...
    <table id="example1" class="table table-bordered table-hover align-middle">
        ...
        <tbody>
            ...
            @foreach ($absen as $k)
                @if ($k->level == 'User')
                    <tr>
                        ...
                        <td class="text-center">
                            @if ($k->status)
                                @if ($k->status == 'pending')
                                    <span style="cursor:pointer;" class="badge bg-danger rounded-0" data-bs-toggle="modal"
                                        data-bs-target="#izinModal{{ $k->id }}">{{ $k->status }}</span>
                                @else
                                    <span class="badge bg-primary rounded-0">{{ $k->status }}</span>
                                @endif
                            @else
                                <span>-</span>
                            @endif
                ...
                @endif
                <div class="modal fade w-100" id="izinModal{{ $k->id }}" tabindex="-1"
                    aria-labelledby="detailModalLabel{{ $k->id }}" aria-hidden="true" data-bs-backdrop="false">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header" style="background-color: #917ECD;">
                                <h5 class="modal-title text-white" id="detailModalLabel{{ $k->id }}">Detail Karyawan
                                </h5>
                            ...
                            <div class="modal-body">
                                <div class="row">
                                    <div class="col-md-12">
                                        <form action="{{ route('terimaizin') }}" method="POST" class="rounded"
                                            style="background-color: #ffffff; border: none; box-shadow: none;"
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="mb-3">
                                                    <label for="alasan" class="form-label">Pengajuan izin</label>
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <button type="button" class="btn btn-secondary border-2"
                                                    style="border-color: #faf7fa; padding: 10px;"
